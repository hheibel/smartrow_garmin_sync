...
class TestSmartRowIntegration(unittest.TestCase):
    ...
    def test_smartrow_download_last_5_tcx(self) -> None:
        ...
        try:
            ...
            self.fail(
                f"Failed to initialize SmartRowClient (ensure GCP secrets are accessible): {e}"
            )
        ...
        activities_sorted = sorted(
            activities, key=lambda x: x.get("created", ""), reverse=True
        )
        ...
        for i, activity in enumerate(last_5_activities):
            ...
            self.assertIsNotNone(
                public_id,
                "Activity is missing 'public_id'. TCX download requires public_id.",
            )
            ...
            self.assertTrue(
                isinstance(tcx_data, bytes) and len(tcx_data) > 0,
                "TCX data should be non-empty bytes",
            )

            # Save locally
            # Clean up timestamp for filename on Windows
            timestamp = (
                activity.get("created", f"activity_{i}")
                .replace(":", "-")
                .replace(".", "-")
            )
        ...
        print(
            f"Successfully downloaded {download_count} TCX files to {output_dir}"
        )
        if len(last_5_activities) > 0:
            self.assertGreater(
                download_count,
                0,
                "Failed to download any TCX files out of the last 5 activities, perhaps they have no TCX data.",
            )
