"""Utility functions for Secret Manager, credentials, and Garmin client."""
...
def access_secret_version(
    ...
    """Accesses the payload for a secret version using GCP Secret Manager.
    ...
    Returns:
        The secret payload as a string, or None if retrieval fails.
    """
    ...
    try:
        ...
        logging.debug(
            "Successfully accessed secret %s with payload length %d.",
            secret_id,
            len(payload),
        )
        return payload
    except Exception as e:
        logging.error("Error retrieving secret %s: %s", secret_id, e)
        return None
...
def parse_credentials_payload(payload: str) -> tuple[str, str]:
    ...
    Args:
        payload: The raw string payload from Secret Manager.
    ...
    for line in payload.splitlines():
        ...
        if line.lower().startswith("username:"):
            # Split only on the *first* colon in case the value itself
            # has colons
...
def read_credentials(credential_secret_id: str) -> tuple[str, str]:
    ...
    if not payload:
        raise RuntimeError(
            f"Could not securely access secret '{credential_secret_id}'. "
            "Ensure you are fully authenticated via "
...
def init_garmin_client() -> Garmin:
    """Initializes and logs into the Garmin client using cached tokens.
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        # Naming it .json ensures the library creates it as a file
        tokenstore = os.path.join(tmp_dir, "garmin_tokens.json")

        # Download if the blob exists on GCS and local file is missing/empty
        if token_blob.exists() and (
            not os.path.isfile(tokenstore) or os.path.getsize(tokenstore) == 0
        ):
            logging.info(
                "Tokenstore does not exist locally, downloading from GCS."
            )
        ...
        try:
            ...
            logging.warning(
                "Token login failed: %s. Falling back to standard login.",
                e,
            )
            ...
            logging.info(
                "Logged in successfully, saving tokens to temp file %s "
                "and to GCS.",
                tokenstore,
            )
