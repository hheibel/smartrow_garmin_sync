...
class TestSyncSmartRowActivities(unittest.TestCase):
    def setUp(self) -> None:
        ...
        if result.returncode != 0:
            self.fail(
                f"gcloud authentication failed or gcloud not found. Please assure you have logged in via gcloud.\nError output:\n{result.stderr}"
            )

        # Setup local mock directory for GCS
        self.mock_gcs_root = os.path.join(
            os.path.dirname(__file__), "mock_gcs_bucket"
        )
    ...
    def test_sync_smartrow_activities_limits_to_10(
        ...
        try:
            ...
            self.fail(
                f"Failed to initialize SmartRowClient or fetch activities: {e}"
            )
        ...
        if len(activities) > 10:
            ...
            state_blob.upload_from_string(
                json.dumps({"last_synced_created": last_synced_timestamp})
            )

            print(
                f"Pre-seeded sync_state.json to exclude all but the last 10 activities (Timestamp cutoff: {last_synced_timestamp})"
            )
        else:
            print(
                "Found 10 or fewer activities on the account, testing without pre-seeding state."
            )
        ...
        self.assertTrue(
            os.path.exists(bucket_dir), "Bucket directory was not created."
        )

        # Look for sync_state.json
        state_file_path = os.path.join(bucket_dir, "sync_state.json")
        self.assertTrue(
            os.path.exists(state_file_path), "sync_state.json should exist"
        )
        ...
        json_files = [
            f for f in files if f.endswith(".json") and f != "sync_state.json"
        ]

        print(
            f"Found {len(json_files)} JSON, {len(tcx_files)} TCX, and {len(fit_files)} FIT files in the mocked GCS bucket."
        )

        self.assertLessEqual(
            len(json_files),
            10,
            "Should have synced at most 10 JSON activities.",
        )
        ...
        with open(state_file_path, encoding="utf-8") as f:
            ...
            self.assertGreater(
                state_data["last_synced_created"],
                activities[-11].get("created", "")
                if len(activities) > 10
                else "",
            )
