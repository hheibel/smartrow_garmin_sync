...
class TestGarminDuplication(unittest.TestCase):
    ...
    def test_verify_duplications(self) -> None:
        ...
        try:
            ...
            print(
                f"Fetched {len(garmin_activities)} recent workouts from Garmin Connect."
            )
        ...
        if os.path.exists(bucket_dir):
            for fname in os.listdir(bucket_dir):
                if fname.endswith(".json") and fname != "sync_state.json":
                    with open(
                        os.path.join(bucket_dir, fname), encoding="utf-8"
                    ) as f:
        ...
        last_30_smartrow = (
            smartrow_activities[-30:]
            if len(smartrow_activities) > 30
            else smartrow_activities
        )
        print(
            f"Evaluating {len(last_30_smartrow)} recent workouts from Mocked GCS...\n"
        )
        ...
        for sr_activity in last_30_smartrow:
            ...
            try:
                sr_dt = datetime.fromisoformat(
                    created_str.replace("Z", "+00:00")
                )
            except ValueError:
                print(
                    f"Could not parse date {created_str} for SR Activity {sr_id}"
                )
                continue
            ...
            if is_dup:
                print(
                    f"[DUPLICATE DETECTED] SmartRow Activity {sr_id} on {created_str} overlaps directly with an existing Garmin workout."
                )
                duplication_count += 1
