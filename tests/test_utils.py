...
class TestUtils(unittest.TestCase):
    ...
    def test_parse_with_extra_whitespace(self):
        payload = "  username:    spaced_user   \n\n\n  password:   secure_pass123   "
        username, password = parse_credentials_payload(payload)
        self.assertEqual(username, "spaced_user")
        self.assertEqual(password, "secure_pass123")

    def test_parse_with_mixed_case_keys(self):
        payload = "UserName: john_doe\nPassWORD: Password1"
        username, password = parse_credentials_payload(payload)
        self.assertEqual(username, "john_doe")
        self.assertEqual(password, "Password1")

    def test_parse_missing_password_raises_error(self):
        payload = "username: test_user"
        with self.assertRaisesRegex(ValueError, "Payload must contain both 'username:' and 'password:' keys."):
            parse_credentials_payload(payload)

    def test_parse_missing_username_raises_error(self):
