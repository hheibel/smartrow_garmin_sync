...
class TestGarminTokenstoreIntegration(unittest.TestCase):
    ...
    def test_init_garmin_client_no_existing_token_uploads_to_gcs(
        self, mock_dump, mock_login, mock_read_credentials
    ):
        ...
        def mock_login_impl(tokenstore=None, *args, **kwargs):
            if tokenstore:
                if (
                    not os.path.exists(tokenstore)
                    or os.path.getsize(tokenstore) == 0
                ):
        ...
        with patch(
            ...
            self.assertTrue(
                blob.exists(),
                "The fallback logic did not upload the token to GCS",
            )
    ...
    @patch("garminconnect.Garmin.login")
    @patch("garminconnect.client.Client.dump")
    def test_init_garmin_client_downloads_existing_token_from_gcs(
        self, mock_dump, mock_login, mock_read_credentials
    ):
        mock_read_credentials.return_value = ("testuser", "testpass")
